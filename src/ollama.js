...
export async function streamGenerate({ model, prompt, onChunk, onDone, onError }) {
  try {
...
export function extractHTML(text) {
  const fullMatch = text.match(/<!DOCTYPE[\s\S]*<\/html>/i) ||
                    text.match(/<html[\s\S]*<\/html>/i);
  if (fullMatch) return fullMatch[0];

  const codeMatch = text.match(/```(?:html)?\s*([\s\S]*?)```/i);
  if (codeMatch) return codeMatch[1].trim();

  if (text.includes('<body') || text.includes('<div') || text.includes('<section')) return text;
...
export function buildPagePrompt({ userPrompt, pageName, siteContext, existingHTML = null, mode = 'create' }) {
  ...
  const baseInstructions = `You are an award-winning web designer/developer building ONE PAGE of a multi-page website. Quality bar: Awwwards/CSS Design Awards portfolio, never generic template-marketplace output.

SITE CONTEXT
- Site/business: ${siteContext?.siteName || 'Untitled site'}
- Overall site description: ${siteContext?.description || userPrompt}
- All pages in this site: ${pagesList}
- You are generating the "${pageName}" page right now — it must share the exact same design identity below as every other page in this site.

${designBrief(designDNA)}

TECHNICAL RULES
- Return ONLY the complete HTML code for THIS page, nothing else
- Include a navigation bar linking to all pages (${pagesList}) styled per the design identity above
- Use inline <style>, Tailwind via CDN (<script src="https://cdn.tailwindcss.com"></script>), Google Fonts via CDN, Font Awesome via CDN
- Fully responsive, production-ready, no placeholder text
- Start with <!DOCTYPE html>`;
